...
for i in range(len(λ)):
    filter.append(np.exp(-((λ[i]-1300e-9)/(1300e-9*0.2))**4))
...
plt.legend()
"""
print('Init Duration with quadratic phase subtracted = {}'.format(rms_width(t1[j0:j1]*1e15, np.abs(Et1[j0:j1])**2)))
print('Init Duration with zero phase = {}'.format(rms_width(t2[j0:j1]*1e15, np.abs(Et2[j0:j1])**2)))
print('Init Duration with of raw data = {}'.format(rms_width(t0[k0:k1]*1e15, np.abs(Et0[k0:k1])**2)))
