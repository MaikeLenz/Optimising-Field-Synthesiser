...
gas="Ne"
pressure_init=(0,9,9,9,9,9,9,9,9,9,9)
radius_init=175e-6
flength_init=1.05
wavel=800e-9
energy_init=3.5e-3
...
inits=50
iters=100
result,iterations=Luna_BO_press(params, initial_values_HCF, function=min_duration, init_points=inits, n_iter=iters)
...
with open('C:\\Users\\ML\\OneDrive - Imperial College London\\MSci_Project\\code\\Synth\\Optimising-Field-Synthesiser\\HCF sim\\Python\\Luna_BO\\tests\\optimise_Luna\\data\\FTminthreshduration'+gas+str(len(pressure_init))+"pressure_points"+'__init_' + str(inits) + '_niter_' + str(iters) + '.csv', 'w', encoding='UTF8', newline='') as f:
