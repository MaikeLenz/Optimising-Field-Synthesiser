...
with open('C:\\Users\\ML\\OneDrive - Imperial College London\\MSci_Project\\code\\Synth\\Optimising-Field-Synthesiser\\HCF sim\\Python\\Luna_BO\\tests\\optimise_Luna\\data\\'+"widthonlyGDD"+'__init_' + str(init_points) + '_niter_' + str(n_iter) + '.csv', 'w', encoding='UTF8', newline='') as f:
    writer = csv.writer(f) # peak_power_1000e-9wavelwindow__init_50_niter_100
    # write the header
    #writer.writerow(header)

    # write the dataS
    writer.writerow([init_points, n_iter, max_width_sum, energy_init, pressure_init, radius_init, flength_init, FWHM_init, wavel, gas, grating_pair_displacement])
