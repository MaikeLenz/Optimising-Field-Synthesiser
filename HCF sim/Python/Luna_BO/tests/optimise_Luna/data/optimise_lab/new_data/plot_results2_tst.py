...
plt.figure(2)
plt.plot(om_slice, phase_slice, '--', label='SPM prediction', color='m')
plt.legend(fontsize=16)
print('RMS width of SPM optimum = {}'.format(rms_width(ω1, Iω1)))
plt.figure(1)
plt.plot(ω1,Iω1, '--', label='SPM prediction', color='m')
plt.fill_between((2*np.pi*c)/(wavel_nm*10**(-9)),max(Iω1)*intens/max(intens),color="lightgray")
