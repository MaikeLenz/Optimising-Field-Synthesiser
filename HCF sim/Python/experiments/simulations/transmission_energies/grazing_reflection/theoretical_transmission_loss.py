...
def theoretical_alpha_um3(lam,gas):
    """
    returns the theoretical decay coefficient from grazing incidence losses in units um^3 /metre
    ...
    if gas=="Ne":
        n_gas=1.000066102
    nu=1.4585/n_gas
    return 2*((2.405/(2*np.pi))**2*((lam**2)/(2))*((nu**2+1)/(np.sqrt(nu**2-1))))*10**18 #in um^3 /m
