...
for i in range(len(Et_out)):
    ...
    thresh_widths.append(threshold(t_out[i],Et_out[i]))
    popt, pcov = curve_fit(superGauss, t[i], Et_out[i])#, p0=[4000, 800, 20])
...
#ax2 = ax1.twinx()
ax1.plot(GDDs*10**30,rms_widths*0.01,label="RMS Width times 0.01",color="tab:blue")
ax1.plot(GDDs*10**30,thresh_widths*0.3,label="Threshold Width times 0.3",color="tab:green")
ax1.plot(GDDs*10**30,superG_widths*0.1,label="Super Gaussian Width times 0.01",color="tab:green")
