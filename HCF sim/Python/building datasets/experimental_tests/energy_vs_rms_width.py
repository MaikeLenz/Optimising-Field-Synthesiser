...
energies = np.linspace(0.8e-3, 1.1e-3, 100)
wavel = 800e-9
...
for i in energies:
    ...
    Main.duv = Main.eval('duv = prop_capillary(radius, flength, gas, (0,pressure); λ0, pulses=pulse, trange=400e-15, λlims=(150e-9, 4e-6))')
...
# Save the data
header = ['Pulse energy, mJ', 'Simulated Angular Frequency Width, /s']
with open('C:\\Users\\iammo\\Documents\\Optimising-Field-Synthesiser\\HCF sim\\Python\\building datasets\\experimental_tests\\simulation_data\\Energy_simulated.csv', 'w', encoding='UTF8', newline='') as f:
    writer = csv.writer(f)
    # write the header
    ...
    for i in range(len(energies)):
        writer.writerow([energies[i]*(10**3), widths[i]])
