...
#GDDs = np.linspace(-100*(10**-30), 100*(10**-30), 20) #GDD in s^2
#TODs = np.linspace(-100*(10**-45), 100*(10**-45), 20) #TOD in s^3
GDDs = np.linspace(-300*(10**-30), 700*(10**-30), 20) #GDD in s^2
TODs = np.linspace(-24000*(10**-45), 46000*(10**-45), 20) #TOD in s^3
Main.energy = energy
Main.λ0 = wavel
...
widths=np.zeros((len(GDDs),len(TODs)))
for i in range(len(GDDs)):
    for j in range(len(TODs)):
        print(i)
        print(j)
        E, ϕω = E_field_freq(omega, GD=0.0, wavel=wavel, domega=domega, amp=1, CEP=0, GDD=GDDs[i], TOD=TODs[j])
        Iω = np.abs(E**2)
        Main.ω = omega
        Main.Iω = Iω  
        Main.phase = ϕω
...
plt.imshow(widths, extent=(np.amin(GDDs)*10**30, np.amax(GDDs)*10**30,np.amin(TODs)*10**45, np.amax(TODs)*10**45), aspect = 'auto', origin="lower")
plt.xlabel("GDD, fs^2")
plt.ylabel("TOD, fs^3")
cbar = plt.colorbar()
...
with open('C:\\Users\\iammo\\Documents\\Optimising-Field-Synthesiser\\HCF sim\\Python\\building_datasets\\custom_input\\data\\GDD_and_TOD_vs_rms_width_large_range.csv', 'w', encoding='UTF8', newline='') as f:
    writer = csv.writer(f)
    # write the header
    ...
    for i in range(len(GDDs)):
        for j in range(len(TODs)):
            writer.writerow([GDDs[i]*10**30, TODs[j]*10**45, widths[i][j]])